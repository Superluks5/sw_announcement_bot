...
if ! systemctl is-active --quiet swbot; then alert "ALERT: swbot.service is offline"; fi
if ! systemctl is-active --quiet swbot-dashboard; then alert "ALERT: swbot-dashboard.service is offline"; fi
if ! curl --fail --silent --max-time 10 "$HEALTH_URL" >/dev/null; then alert "ALERT: dashboard health check failed"; fi

restarts="$(systemctl show swbot --property=NRestarts --value)"
...
oauth_failures="$(journalctl -u swbot-dashboard --since '10 minutes ago' --no-pager 2>/dev/null | grep -Ec 'OAuth .*failed|OAuth callback rejected|OAuth token exchange failed' || true)"
